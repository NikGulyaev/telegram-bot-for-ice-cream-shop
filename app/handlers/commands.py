...
async def create_order(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    order_service: OrderService = context.application.order_service #type: ignore[attr-defined]
    product_service: ProductService = context.application.product_service #type: ignore[attr-defined]
...
async def finish_order(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    waiter_users_ids = await user_service.get_waiter_users_ids()
    order = await order_service.get_order_by_id(order_id)
        ...
        await context.bot.send_message(chat_id=waiter_user_id,
                                       ...
                                       reply_markup=InlineKeyboardMarkup([
                                           [InlineKeyboardButton("Заказ доставлен",
                                                                 callback_data=("waiter_finish_order"))]
                                       ]))